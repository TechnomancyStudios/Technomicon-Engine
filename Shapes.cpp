...
Rectangle::Rectangle(glm::vec2 xy1, glm::vec2 xy2)
{
	this->xy1 = xy1;
	this->xy2 = xy2;
...
void Rectangle::Draw(Shader shader, glm::mat4 m_view, glm::mat4 m_projection)
{
	//float yeet = (xy2.y - xy1.y) / (xy2.x - xy1.x);
	//float angle = glm::atan(yeet);
	glm::mat4 final_trans = glm::mat4(1.0f);
	...
	m_world = glm::translate(m_world, glm::vec3(-(xy1.x - (glm::cos(rad) * origin.x - glm::sin(rad)*origin.y)), -(xy1.y - (glm::cos(rad) * origin.y + glm::sin(rad)*origin.x)), 1.0f));
	//float sum = xy2.x + glm::abs(xy2.y);
	m_world = glm::rotate(m_world, glm::radians(angle), glm::vec3(0.0f, 0.0f, 1.0f));
	float x_scale = (xy2.x - xy1.x);
	float y_scale = (xy2.y - xy1.y);
	m_world = glm::scale(m_world, glm::vec3(-x_scale, -y_scale, 1.0f));
